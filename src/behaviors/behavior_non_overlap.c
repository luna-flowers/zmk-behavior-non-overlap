 ...
 * The array size `active_count` is the number of key presses the behavior can remember.
 * `active_count = resume-capacity + 1`.
 * `active_count = 10` if `resume-capacity` is not specified.
 * `active_count = 1` if `no-resume` is set.
...
static inline struct active_non_overlap *find_empty_slot(struct behavior_non_overlap_data *data) {
    const size_t active_count = data->active_count;
...
static int on_non_overlap_binding_released(struct zmk_behavior_binding *binding,
    ...
    if (active == most_recent_active) {
        ...
        if (most_recent_active != NULL) {
            LOG_DBG("Previous active exists. Resume it.");
...
#define _RESUME_CAPACITY(inst)                                                                     \
    COND_CODE_1(DT_INST_PROP(inst, no_resume), (0), (DT_INST_PROP_OR(inst, resume_capacity, 9)))

#define _EMPTY_ARRAY(inst)                                                                         \
    {_EMPTY_ACTIVE(, inst), LISTIFY(_RESUME_CAPACITY(inst), _EMPTY_ACTIVE, (, ), inst)}
...
#define KP_INST(inst)                                                                              \
    ...
    static struct active_non_overlap actives_##inst[] = _EMPTY_ARRAY(inst);                        \
    static struct behavior_non_overlap_data behavior_non_overlap_data_##inst = {                   \
        .head = NULL,                                                                              \
        .tail = NULL,                                                                              \
        .actives = actives_##inst,                                                                 \
        .active_count = _RESUME_CAPACITY(inst) + 1,                                                \
    };                                                                                             \
